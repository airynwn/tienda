...
class Factura extends Modelo
{
    ...
    public $created_at;
    public $usuario_id;
    private $total; // importe total de la factura
    ...
    public function getTotal(?PDO $pdo = null)
    {
        $pdo = $pdo ?? conectar();
        // (importe total)
        if (!isset($this->total)) { // col con suma del precio de cada articulo * su cantidad
    ...
    public static function todosConTotal( //? mira que hace exactamente
        array $where = [],
        array $execute = [],
        ?PDO $pdo = null
    ): array
    ...
    {  // basicamente get factura segun su id y crear las filas como lineas
        $pdo = $pdo ?? conectar();
